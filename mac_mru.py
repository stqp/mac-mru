...
class Mru:
    def __init__(self, evidence_root, out_path):
    ...
    def load_bplist(self, path):
        try:
            with open(path, "rb") as plistfile:
               return ccl_bplist.load(plistfile)
        except:
            return None
    ...
    def parse(self):
        for root, dirs, filenames in os.walk(self.evidence_root):
            for f in filenames:
                ...
                fmts = []
                if self.isMatch(f,['*.sfl']) and not self.isMatch(f,['*Favorite*.sfl','*Project*.sfl','*iCloudItems*.sfl'], orMatch=True):
                    fmts = self.format_bookmarks(self.parseSFL(mru_file))

                elif self.isMatch(f,['*.sfl2']) and not self.isMatch(f, ['*Favorite*.sfl2','*Project*.sfl2','*iCloudItems*.sfl2'], orMatch=True):
...
if __name__ == '__main__':
    ...
    Mru(args.root, args.output).parse()
